...
-- MariaDB
DROP DATABASE IF EXISTS `community_user_db`;
CREATE DATABASE `community_user_db` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;

USE `community_user_db`;

DROP TABLE IF EXISTS `user`;

CREATE TABLE `user` (
    `user_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(32) NOT NULL UNIQUE COMMENT '고객명',
    `email` VARCHAR(255) NOT NULL UNIQUE COMMENT '고객 이메일',
    `password` VARCHAR(255) NOT NULL COMMENT '고객 비밀번호',
    `created_at` DATETIME NOT NULL COMMENT '회원가입일',
    `deleted_at` DATETIME NULL COMMENT '탈퇴일',
    `profile_id` INT UNSIGNED NULL COMMENT '프로필 사진 아이디',
    PRIMARY KEY (`user_id`)
